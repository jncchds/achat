...
export interface LlmInteractionDto {
  id: string; botId: string | null; botName: string | null;
...
export const usageApi = {
  ...
  getBotUsage: (botId: string, page = 1, pageSize = 20) =>
    api.get<LlmUsagePagedResult>(`/bots/${botId}/usage?page=${page}&pageSize=${pageSize}`).then(r => r.data),
};
